...
bst_t *bst_delete(bst_t *root, bst_t *node)
{
	...
	/* akunangane noma ingane yangakwesokudla kuphela */
	if (node->left == NULL)
	{
	...
	/* ingane yangakwesokunxele kuphela */
	if (node->right == NULL)
	{
		if (parent != NULL && parent->left == node)
...
/**
 * bst_remove_recursive - isusa i-node kwi binary search tree recursively.
 * @root: A pointer to the root node of the BST to remove a node from.
 * @node: A pointer kwi current node in the BST.
 * @value: ivelu yokususa kwi BST.
 *
